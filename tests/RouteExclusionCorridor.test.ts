...
describe('RouteExclusionCorridor', () => {
  ...
  it('enforces strengthened horizontal clearance on standard platforms', () => {
    // Node 1 is a standard platform at x=0, z=0 with trackHalfBreadth = 6m.
    // Strengthened margin: 36.0 + 0.65 * objectRadius (radius 2m -> 37.3m)
    // Required distance: 6 + 2 + 37.3 = 45.3m
    const closePos = new THREE.Vector3(22, 0, 0); // 22m away horizontally (rejected)
    const safePos = new THREE.Vector3(130, 0, 0); // far outside every margin
  ...
  it('enforces stair / ascent section clearance (42.0m base margin)', () => {
    ...
    // Node 1: trackHalfBreadth 6m, radius 2m. Margin: 42.0 + 0.7 * 2 = 43.4m. Required: 51.4m.
    const insideStepUp = new THREE.Vector3(38, 0, 0);
    const outsideStepUp = new THREE.Vector3(90, 0, 0);
  ...
  it('protects the full flared exit width of ascent platforms', () => {
    ...
    expect(flaredCorridor.isPointInsideCorridor(new THREE.Vector3(100, 0, 0), 2, -5, 10)).toBe(false);
  });

  it('enforces strengthened lateral clearance on surf sections', () => {
    // Node 2 is a surf ramp with dimensions.x = 14 (half-breadth 7m)
    // Surf margin: Math.max(50.0, 26.0 + extraSurfMargin) + 0.85 * objectRadius
    // For radius 5m: 50.0 + 4.25 = 54.25m -> Required: 7 + 5 + 54.25 = 66.25m
    const closeSurfPos = new THREE.Vector3(36, -5, 50); // 36m away horizontally
    const farSurfPos = new THREE.Vector3(120, -5, 50);  // far outside the surf corridor
  ...
  it('rejects an object placed directly in the jump flight path between nodes', () => {
    ...
    // Lateral clearance on the jump flight path with a surf transition.
    const nearFlightPos = new THREE.Vector3(35, -2.5, 25);
    const farFlightPos = new THREE.Vector3(110, -2.5, 25);
  ...
  it('classifies vertical intrusion, surf corridor and comfort rejections separately', () => {
    ...
    // 2. Tall structure spanning the whole protected band beside the route:
    //    a vertical intrusion rather than a plain comfort violation.
    const spanningBox = new THREE.Box3(
      new THREE.Vector3(40, -100, -10),
      new THREE.Vector3(70, 100, 10)
    );
    expect(corridor.isBoxInsideCorridor(spanningBox)).toBe(true);

    // 3. Low structure beside the route inside the comfort band: comfort only.
    const lowBox = new THREE.Box3(
      new THREE.Vector3(40, -3, -10),
      new THREE.Vector3(70, 2, 10)
    );
    expect(corridor.isBoxInsideCorridor(lowBox)).toBe(true);

    // 4. Clean surviving candidate: distant tower far outside every margin.
    const safeBox = new THREE.Box3(
      new THREE.Vector3(300, -100, -10),
      new THREE.Vector3(330, 100, 10)
    );
    expect(corridor.isBoxInsideCorridor(safeBox)).toBe(false);

    const report = RouteExclusionCorridor.getLastBuildingReport();
    expect(report.candidatesGenerated).toBe(4);
    expect(report.rejectedByGameplayCollision).toBe(1);
    expect(report.rejectedByVerticalIntrusion).toBe(1);
    expect(report.rejectedByComfortClearance).toBe(1);
    expect(report.finalSurvivingBuildings).toBe(1);
  });

  it('classifies surf corridor intrusions separately', () => {
    RouteExclusionCorridor.resetBuildingReport();
    // Straight ahead of the surf ramp exit, inside the launch cone.
    const coneBox = new THREE.Box3(
      new THREE.Vector3(-6, -5, 130),
      new THREE.Vector3(6, 25, 142)
    );
    const hit = corridor.evaluateVolume(coneBox);
    expect(hit).not.toBeNull();
    expect(hit!.kind).toBe('SURF_CORRIDOR');
    expect(corridor.isBoxInsideCorridor(coneBox)).toBe(true);
    expect(RouteExclusionCorridor.getLastBuildingReport().rejectedBySurfCorridor).toBe(1);
  });
});
